...
Uses streamlit-webrtc for browser-based camera access (works on Streamlit Cloud).
"""
...
st.markdown(
    ...
    <div style="margin-bottom: 1.5rem;">
        ...
        <p style="color: #5A5D5A; font-size: 1.05rem;">
            Select <strong>Start Practice</strong> and allow camera access when prompted. HastaAI will analyze your gesture in real time.
        </p>
    </div>
    """,
...
if st.session_state.is_running:
    try:
        ...
        # Lazy-load ML models into session state (persists across rerenders)
        ...
        # Shared result container accessed from callback thread
        if "webrtc_result" not in st.session_state:
            st.session_state.webrtc_result = None

        class _MudraProcessor(VideoProcessorBase):
            """Processes each webcam frame: detects hand, classifies mudra, annotates frame."""

            def __init__(self):
                self.detector = st.session_state.ml_detector
                self.extractor = st.session_state.ml_extractor
                self.classifier = st.session_state.ml_classifier
                self.frame_idx = 0

            def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
                import cv2 as _cv2
                bgr = frame.to_ndarray(format="bgr24")
                bgr = _cv2.flip(bgr, 1)  # Mirror view

                results = self.detector.detect(bgr)
                annotated = self.detector.draw(bgr, results)
                hand = self.extractor.extract(results, bgr.shape)
                ...
                if hand:
                    ...
                    # Log every 5 frames and store result
                    if self.frame_idx % 5 == 0 and st.session_state.session_id:
                        m_row = get_mudra_by_name_db(result.name)
                        m_id = m_row["id"] if m_row else None
                        log_recognition_result(st.session_state.session_id, m_id, result.confidence)
                        st.session_state.session_detections.append({
                            "name": result.name,
                            "confidence": result.confidence,
                            "detected": result.detected,
                        })
                    st.session_state.webrtc_result = result
                else:
        ...
        # STUN servers for WebRTC NAT traversal (required for cloud)
        ...
        # Live result sidebar
        live_result = st.session_state.get("webrtc_result")
        if live_result:
            if live_result.status_label == "Excellent Match":
                badge_cls = "badge-excellent"
            elif live_result.status_label == "Good Match":
                badge_cls = "badge-good"
            else:
                badge_cls = "badge-adjusting"

            low_conf_html = ""
            if live_result.confidence < 0.65:
                low_conf_html = """
            ...
            result_box.markdown(
                ...
                <div class="hasta-card">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                        <span class="badge-status {badge_cls}">{live_result.status_label}</span>
                        <span style="font-size: 0.85rem; color: #C5A059; font-weight: 700;">{live_result.sanskrit}</span>
                    </div>
                    <div class="card-title" style="font-size: 1.8rem; margin: 0.2rem 0;">{live_result.name}</div>
                    <div style="margin: 0.8rem 0;">
                        <div style="display: flex; justify-content: space-between; font-size: 0.85rem; margin-bottom: 0.3rem;">
                            <span style="font-weight: 600; color: #722F37;">Match Confidence</span>
                            <span style="font-weight: 700; color: #722F37;">{live_result.confidence:.0%}</span>
                        </div>
                        <div style="width: 100%; background: #F3EFEA; height: 10px; border-radius: 5px; overflow: hidden;">
                            <div style="width: {int(live_result.confidence * 100)}%; background: linear-gradient(90deg, #722F37, #C5A059); height: 100%; transition: width 0.3s ease;"></div>
                        </div>
                    </div>
                    <div class="card-desc" style="margin-top: 0.8rem;">
                        <strong>Description:</strong> {live_result.description}
                    </div>
                    <div class="card-desc" style="margin-top: 0.5rem;">
                        <strong>Significance:</strong> {live_result.significance}
                    </div>
                    <div style="margin-top: 0.8rem; padding: 0.8rem; background: #FAF8F5; border-radius: 8px; border-left: 3px solid #C5A059; font-size: 0.85rem; color: #444;">
                        <strong>Practice Guidance:</strong> {live_result.instructions}
                    </div>
                    {low_conf_html}
                </div>
                """,
        ...
        st.error(
            ...
            "Please add `streamlit-webrtc` to your requirements.txt and redeploy."
        )
